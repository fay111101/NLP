...
def get_train_vecs(x_train,x_test):
    ...
    np.save(model_path+'train_vecs.npy',train_vecs)
    print(train_vecs.shape)
    #Train word2vec on test tweets
    imdb_w2v.train(x_test)
    imdb_w2v.save(model_path+'w2v_model.pkl')
    #Build test tweet vectors then scale
    test_vecs = np.concatenate([buildWordVector(z, n_dim,imdb_w2v) for z in x_test])
    #test_vecs = scale(test_vecs)
    np.save(model_path+'test_vecs.npy',test_vecs)
    print(test_vecs.shape)



def get_data():
    train_vecs=np.load(model_path+'train_vecs.npy')
    y_train=np.load(data_path+'y_train.npy')
    test_vecs=np.load(model_path+'test_vecs.npy')
...
def get_predict_vecs(words):
    n_dim = 300
    imdb_w2v = Word2Vec.load(model_path+'w2v_model.pkl')
